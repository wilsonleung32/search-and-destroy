...
describe('isLoop function', () => {
  ...
  it('should return true single node loops to self', () => {
    const list = new LinkedList();
    list.addToHead(1);
    list.head.next = list.head;
    expect(isLoop(list)).to.equal(true);
  });
  it('should return false single node does not loop', () => {
    const list = new LinkedList();
    list.addToHead(1);

    expect(isLoop(list)).to.equal(false);
  });
  it('should return false if no nodes in linkedlist', () => {
    const list = new LinkedList();
